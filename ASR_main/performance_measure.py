...
def dtw_torch(x, y, dist, device, warp = 1, w = 'inf'):
    ...
    if T1 > T2:
        ...
        T1, T2 = T2, T1
    # always T1 < T2, to accelerate calculation
    cost_matrix_ = torch.zeros((T1+1, T2+1), device=device)
    cost_matrix_[1:, 0] = float('inf')
    cost_matrix_[0, 1:] = float('inf')
    cost_matrix = cost_matrix_[1:, 1:] # View
    ...
    # 2] Compute accumulated cost path(minimum cost matrix) & minimum cost path (T2 ~ T1+T2-1)
    # Accumulate to existing cost_matrix
    path_matrix = torch.zeros((T1, T2, 2), dtype=int)
    x_i = {0:-1, 1:0, 2:-1}
    y_i = {0:-1, 1:-1, 2:0}
    for i in range(T1):
        for j in range(T2):
    ...
    if swap == True:
        x_path, y_path = y_path, x_path
    return cost_matrix[-1,-1].cpu().numpy(), [x_path, y_path]
