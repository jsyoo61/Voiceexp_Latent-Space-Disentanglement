...
def dtw_torch(x, y, dist, device, warp = 1, w = 'inf'):
    '''Dynamic Time Warping of two sequences, accelerated with
    matrix operations instead of sequential vector operations
    ...
    cost_matrix_[1:, 0] = float('inf')
    cost_matrix_[0, 1:] = float('inf')
    # cost_matrix = torch.as_tensor(cost_matrix_[1:, 1:], device=device)
    cost_matrix = torch.zeros((T1,T2), device=device)

    # 1] Get cost matrix: dist(vector, matrix) -> Faster calculation
    # Scalar sequences
    if x.ndim == 1:
        # cost_matrix[:, :] = dist(np.expand_dims(x,-1), np.broadcast_to(y, (T1,T2)))
        cost_matrix[:, :] = dist(torch.unsqueeze(x, -1), y.expand(T1, -1))
        # cost_matrix[:, :] = dist(x - y.expand(T1, -1).T).T
    # Vector sequences
    else:
        for i, vector in enumerate(x):
            cost_matrix[i] = dist(vector, y)
    # cost_matrix_base = cost_matrix.copy() # To save cost matrix
    cost_matrix_[1:,1:] = cost_matrix.cpu().numpy()
    cost_matrix = cost_matrix_[1:,1:] # View
    # et = time.time()
    # print(et - st)
...
def mcd_cal_torch(converted_mcep, target_mcep):
    converted_mcep = converted_mcep[:,1:]
    target_mcep = target_mcep[:,1:]
    distance, path = dtw_torch(converted_mcep, target_mcep, dist=mcd_vectorized_torch)
    T = len(path[0])
    mcd = distance / T
    return mcd
...
#
# st = time.time()
# result3 = dtw(x,y, dist=mcd_vectorized)
# et = time.time()
# print(et - st)
#
# st = time.time()
# result4 = dtw_torch(x,y, dist=mcd_vectorized_torch)
# et = time.time()
# print(et - st)
#
# # %%
