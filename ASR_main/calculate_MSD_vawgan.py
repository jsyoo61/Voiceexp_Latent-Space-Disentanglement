...
result_file_mfcc = "MSD_result_vae.txt"
...
def calculateMCD_manual_mfcc(target_converted_file_path, compare_original_file_key, model_epoch, target_converted_source_id, compare_original_target_id):
    if target_converted_source_id == compare_original_target_id:
      ...
      target_conv_wav_mfcc = np.fft.fft(target_conv_wav_mfcc.T,512)
      ...
      compare_ori_wav_mfcc = np.fft.fft(compare_ori_wav_mfcc.T,512)
      MSD = np.mean(np.abs(np.sqrt(np.power((target_conv_wav_mfcc-compare_ori_wav_mfcc),2))))
      resultLine = "epoch: " + model_epoch + " file: " + pure_file_name + ' ' + target_converted_source_id + " -> " + compare_original_target_id + " MSD: " + str(MSD)+"\n"
      print("{} {} {} {} {}".format(model_epoch, pure_file_name, target_converted_source_id, compare_original_target_id,str(MSD)))
      return resultLine
...
for each_dir_conv_way in os.listdir(test_dir):
        ...
        for each_file in os.listdir(os.path.join(test_dir, each_dir_conv_way)):
            if ".wav" in each_file: #[spk_src]_to_[psk_trg]_[spk_id_sn]_[file_name].wav
                # ex) F2_to_M2_fy03_t10_s23.wav
                """
                fileName1, fileName2,fileName3 = each_file.split("-")
                remain_file_name = fileName3
                
               
                pure_file_name = remain_file_name.split('.')[0]#del spk_id_sn
                """
                pure_file_name = each_file.split('.')[0]#del spk_id_sn
