...
def conversion(model_dir, source, target,model_iter):
    for source in range(4):
        for target in range(4):
          if source!=target:
            ...
            #src_speaker = source
            #trg_speaker = target
            ppg_dir = os.path.join('processed_ppgs_test')
            exp_ppg_dir =os.path.join(ppg_dir,src_speaker)
            ...
            #model_name, model_number = model_dir.split("-")
            #model_name= model_name+model_iter
            ...
            src_c[int(src_num)] = 1.0
            trg_c[int(trg_num)] = 1.0
            
            print(trg_c)
            src_c = torch.Tensor(src_c).to(device, dtype=torch.float)
            trg_c = torch.Tensor(trg_c).to(device, dtype=torch.float)
            """
            ...
            for file in flist:
                        ...
                        _, ap, f0 = load_pickle(os.path.join(exp_test_dir, '{}.p'.format(file.split('/')[-1].split('.')[0])))
                        sp = load_pickle(
                                        os.path.join(exp_ppg_dir, '{}.p'.format(file.split('/')[-1].split('.')[0])))
                        ...
                        #coded_sp_converted_norm = sess.run(x_t, feed_dict={x_test:np.array([coded_sp_norm]),y_A:speaker_A,y_B:speaker_B})[0]
...
if __name__ == '__main__':
      ...
      parser.add_argument('--model_iter', type=str, help='model iteration number', default=model_iter)
      ...
      conversion(model_dir = model_dir, source=source_speaker, target=target_speaker,model_iter=model_iter)
