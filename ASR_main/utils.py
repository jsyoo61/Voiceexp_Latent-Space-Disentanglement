...
def calculate_gradients_penalty(netD, real_data, fake_data):
    ...
    alpha = alpha.expand(Batch_size, real_data.nelement()//Batch_size).contiguous().view(Batch_size, 1, 36, 128)
    ...
    gradients_penalty = torch.mean(torch.sum(gradients_penalty))
